...
bool amende::ajouter()
{
    ...
    QString res= QString::number(id);
     query.prepare("INSERT INTO AMENDE(CIN_A,NOM,PRENOM,TYPE,MONTANT,DATEA)""VALUES(:id,:nom,:prenom,:typeA,:montant,:datea)") ;
...
bool amende::supprimer(int id)
{
    QSqlQuery  query;
    QString res= QString::number(id);
     query.prepare("Delete from AMENDE where CIN_A = :id");
...
QSqlQueryModel *amende::rechercher(int id)
{
    ...
    model1->setQuery("select * from AMENDE where CIN_A LIKE '"+res+"%'");
...
bool amende::modifier(int id)
{
    QSqlQuery  query;
    QString res= QString::number(id);
     query.prepare("update AMENDE set CIN_A = :id, NOM = :nom, PRENOM = :prenom, TYPE = :typeA, MONTANT = :montant, DATEA = :datea where CIN_A = :id") ;
     query.bindValue(":id",res);
     query.bindValue(":nom",nom);
     query.bindValue(":prenom",prenom);
     query.bindValue(":typeA",typeA);
