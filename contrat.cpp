...
bool contrat::modifierContrat(int idd)
{

    QSqlQuery query;
    QString res=QString::number(idd);
    QString cinp1=QString::number(cinp);
    query.prepare("UPDATE TABLE1 set cinm=:id,nomm=:nomprenomm,metierm=:metierm,nomp=:nomprenomp,metierp=:metierp,cinp=:cinp,date=:datemariage where cinm=:id ");
    query.bindValue(":nomp",nomp);
    query.bindValue(":nomm",nomm);
    query.bindValue(":metierp",metierp);
     query.bindValue(":metierm",metierm);
