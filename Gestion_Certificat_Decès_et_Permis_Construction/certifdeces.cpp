...
bool Certifdeces::ajouter_certif()
{
    ...
    query.prepare("INSERT INTO CERTIFDECES(CIN, NOM, PRENOM, DATEDECES, LIEU, ETATMATRIM)"
...
QSqlQueryModel * Certifdeces::afficher_certif()
{
 ...
 bool Certifdeces::supprimer_certif(int idc)
 {
     ...
     query.prepare("delete from Certifdeces where CIN= :id");
 ...
 bool Certifdeces::modifier_certif(int idd) //modifier certif
 {
     ...
     query.prepare("Update certifdeces set CIN = :id , NOM = :nom , PRENOM = :prenom , DATEDECES = :datedeces , LIEU = :lieu , ETATMATRIM = :etatmatrim  where CIN = :id ");
     query.bindValue(":id", res);
 ...
 QSqlQueryModel *Certifdeces::recherche(QString cin)
   {
       QSqlQueryModel * model= new QSqlQueryModel();
       model->setQuery("select * from CERTIFDECES where cin LIKE '"+cin+"%' or nom LIKE '"+cin+"%' or prenom LIKE '"+cin+"%'");
